...
namespace RenderLib
{
  namespace CPU
  {
    namespace Texture
    {
      ...
      public:
        virtual void
        configureBlockConfig(TextureLoadResult * src,
      ...
      private:
        ...
        std::unordered_map<TextureType,
                           std::unique_ptr<AbstractTextureBlockConfigSetup>>
            blockSetups;
        std::unordered_map<TextureStackBlendOperation,
                           std::unique_ptr<TextureBlendOperation>>
            blendOperations;
      ...
      public:
        ...
        void
        registerTextureTypeConfigurator()
        {
          if (std::is_base_of<AbstractTextureBlockConfigSetup, T>::value)
          {
            std::unique_ptr<AbstractTextureBlockConfigSetup> newConfigurator
                = std::make_unique<T>();
          ...
          else
          {
            Logger::Log::getInstance().logWarning(
                "TextureManager: Tried to register block config setup class "
                "non "
                "derived from AbstractTextureBlockConfigSetup");
          }
        }

        template <TextureStackBlendOperation bo, class T>
        void
        registerTextureBlendOperation()
        {
          if (std::is_base_of<TextureBlendOperation, T>::value)
          {
            std::unique_ptr<TextureBlendOperation> newOperation
                = std::make_unique<T>();
          ...
          {
            Logger::Log::getInstance().logWarning(
                "TextureManager: Tried to register blend operation class non "
                "derived "
                "from TextureBlendOperation");
          }
        }

        Texture *
        loadTexture(const std::string & file, TextureType type);

        Texture *
        loadAndProcessTextureStack(
            const std::string & name, VECTOR3 baseColor,
            std::vector<IO::LoadedParameter<Mesh::MaterialTextureInfo>> &
                stack);
      ...
      private:
        void
        loadTempTexture(const std::string & file, TextureType type,
                        std::vector<unsigned char> & buf, unsigned int & width,
                        unsigned int & height);
      };
    } // namespace Texture
  } // namespace CPU

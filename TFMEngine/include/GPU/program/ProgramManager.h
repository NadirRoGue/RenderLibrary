  ...
  namespace GPU
  {
    namespace Program
    {
      ...
      public:
        ...
        createProgram(UberParamMask mask) = 0;
      };
      ...
      public:
        ...
        createProgram(UberParamMask mask)
        {
      ...
      private:
        static std::unordered_map<std::type_index,
                                  std::unique_ptr<AbstractProgramFactory>>
            programFactories;
        std::unordered_map<
            std::type_index,
            std::unordered_map<UberParamMask, std::unique_ptr<Program>>>
            programList;
      ...
      public:
        ...
        registerProgram()
        {
          ...
          {
            throw EngineException("ProgramManager: Attempted to register a "
                                  "non-derived Program program class");
            return;
          }

          std::type_index id = typeid(T);
          std::unique_ptr<AbstractProgramFactory> newFactory
              = std::make_unique<ProgramFactory<T>>();
          programFactories[id] = std::move(newFactory);
        }

        template <class T>
        T *
        getProgram(const UberParamMask configMask = 0)
        {
          std::type_index id = typeid(T);
          ...
          if (!std::is_base_of<Program, T>::value)
          {
            throw new EngineException(
                "ProgramManager: Attempted to create a program from the non "
        ...
        Program *
        getProgram(const UberParamMask configMask,
                   std::type_index programTypeId);
      };
    } // namespace Program
  } // namespace GPU
} // namespace RenderLib

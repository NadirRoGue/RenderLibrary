...
namespace RenderLib
{
  namespace Lighting
  {
    ...
    template <class T> class Light : public SceneObject
    {
    private:
    ...
    public:
      Light() : SceneObject()
      {
        ...
        {
          throw EngineException("Light: Attempted to instantiate a light with "
                                "a non LightData derived light data struct");
        }
      }
      ...
      void
      setLightColor(const VECTOR3 & color)
      {
      ...
      void
      setDiffuseIntensity(const float & value)
      {
        lightData.get()->lightKd = value;
      }

      void
      setSpecularIntensity(const float & value)
      {
        lightData.get()->lightKs = value;
      }

      void
      setAmbientIntensity(const float & value)
      {

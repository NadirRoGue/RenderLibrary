...
directionalLightShading()
{
  ...
  for (int i = 0; i < MAX_DIRECTIONAL_LIGHTS; i++)
  {
    ...
    c += dlclr * dirLights[i].dlks * Ks * pow(DL_Factor, shininessVal)
        * specularScaleVal;
  }
...
pointLightShading()
{
  ...
  for (int i = 0; i < MAX_POINT_LIGHTS; i++)
  {
    ...
    float PL_fatt = min(
        1 / (platt.x + platt.y * PL_dist + platt.z * PL_dist * PL_dist), 1);
    ...
    // DIFFUSE
    c += clamp(plclr * pointLights[i].plkd * Kd
                   * max(dot(normalVector, pldir), 0),
               0, 1)
        * PL_fatt;
    ...
    c += clamp(plclr * pointLights[i].plks * Ks * pow(PL_Factor, shininessVal)
                   * specularScaleVal,
               0, 1)
        * PL_fatt;
  }
...
spotLightShading()
{
  ...
  for (int i = 0; i < MAX_SPOT_LIGHTS; i++)
  {
    ...
    float SL_fatt = min(
        1 / (slatt.x + slatt.y * SL_dist + slatt.z * SL_dist * SL_dist), 1);

    vec3 sldir = normalize(slpos - pos);

    float cosapperture = cos(radians(apperture));
    float deviation    = dot(slconedir, -sldir);
    float spotfactor   = max(
        pow(max(deviation - cosapperture, 0) / (1 - cosapperture), theta), 0);
    ...
    // DIFFUSE
    c += clamp(spotfactor
                   * (slclr * spotLights[i].slkd * Kd
                      * max(dot(normalVector, sldir), 0))
                   * SL_fatt,
               0, 1);
    ...
    c += clamp(spotfactor * slclr * spotLights[i].slks * Ks
                   * pow(SL_Factor, shininessVal) * specularScaleVal * SL_fatt, 0, 1);
  }

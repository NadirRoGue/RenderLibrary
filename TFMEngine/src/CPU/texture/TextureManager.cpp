...
namespace RenderLib
{
  namespace CPU
  {
    ...
    {
      void
      Texture1DBlockConfigSetup::configureBlockConfig(
          TextureLoadResult * src, TextureBlockConfiguration & config)
      {
      ...
      void
      Texture2DBlockConfigSetup::configureBlockConfig(
          TextureLoadResult * src, TextureBlockConfiguration & config)
      {
        config.width     = src->width;
        config.height    = src->height;
      ...
      void
      Texture3DBlockConfigSetup::configureBlockConfig(
          TextureLoadResult * src, TextureBlockConfiguration & config)
      {
        config.width     = src->width;
      ...
      {
        registerTextureTypeConfigurator<TextureType::TEXTURE1D,
                                        Texture1DBlockConfigSetup>();
        registerTextureTypeConfigurator<TextureType::TEXTURE2D,
                                        Texture2DBlockConfigSetup>();
        registerTextureTypeConfigurator<TextureType::TEXTURE3D,
                                        Texture3DBlockConfigSetup>();

        registerTextureBlendOperation<TextureStackBlendOperation::OP_ADD,
                                      BlendAdd>();
        registerTextureBlendOperation<TextureStackBlendOperation::OP_DIVIDE,
                                      BlendDivide>();
        registerTextureBlendOperation<TextureStackBlendOperation::OP_MULTIPLY,
                                      BlendMultiply>();
        registerTextureBlendOperation<TextureStackBlendOperation::OP_SIGNEDADD,
                                      BlendSignedAdd>();
        registerTextureBlendOperation<TextureStackBlendOperation::OP_SMOOTHADD,
                                      BlendSmoothAdd>();
        registerTextureBlendOperation<TextureStackBlendOperation::OP_SUBSTRACT,
                                      BlendSub>();
      }
      ...
      Texture *
      TextureManager::loadTexture(const std::string & name, TextureType type)
      {
        ...
        IO::AbstractLoadResultPtr loadResult
            = IO::FileManager::loadFile(name, 0);
        TextureLoadResult * texLoadResult
            = static_cast<TextureLoadResult *>(loadResult.get());
        ...
        if (it == blockSetups.end())
        {
          ...
          throw EngineException(
              "TextureManager: Could not find a block configurator for texture "
              + name);
          return NULL;
        }
        ...
        Memory::MemoryManager::getInstance().configureObject<Texture>(
            newTexture.get(), &config);
      ...
      Texture *
      TextureManager::loadAndProcessTextureStack(
          const std::string & name, VECTOR3 baseColor,
          std::vector<IO::LoadedParameter<Mesh::MaterialTextureInfo>> & stack)
      {
        ...
        loadTempTexture(info.filePath, TextureType::TEXTURE2D, base, width,
                        height);

        std::vector<unsigned char> baseColorBuf;
        baseColorBuf.resize(width * height * 4);
        baseColor.normalize();
        VECTOR3 bc(baseColor.x() * (FLOAT)255.0, baseColor.y() * (FLOAT)255.0,
                   baseColor.z() * (FLOAT)255.0);
        ...
        {
          it->second.get()->doBlend(baseColorBuf, base,
                                    baseParam.get().blendStrength);
        }
        else
        {
          Logger::Log::getInstance().logWarning(
              "TextureManager: Textue on st ack of " + name
              + " uses unknown blend operation. Skipping");
        }
        ...
        for (; i < stack.size(); i++)
        {
          ...
          if (param.paramExists())
          {
            ...
            if (blendOpIt != blendOperations.end())
            {
              ...
              loadTempTexture(param.get().filePath, TextureType::TEXTURE2D,
                              nextOnStack, w, h);

              if (nextOnStack.size() == baseColorBuf.size())
              {
                blendOpIt->second.get()->doBlend(baseColorBuf, nextOnStack,
                                                 param.get().blendStrength);
              }
              else
              {
                Logger::Log::getInstance().logWarning(
                    "TextureManager: Texture on stack of " + name
                    + " has different size than base texture. Skipping");
              }
            }
            else
            {
              Logger::Log::getInstance().logWarning(
                  "TextureManger: Texture on stack of " + name
                  + " uses unknown blend operation. Skipping");
            }
          }
        }
      ...
      void
      TextureManager::loadTempTexture(const std::string & file,
                                      TextureType type,
      ...
      {
        IO::AbstractLoadResultPtr loadResult
            = IO::FileManager::loadFile(file, 0);
        TextureLoadResult * texLoadResult
            = static_cast<TextureLoadResult *>(loadResult.get());

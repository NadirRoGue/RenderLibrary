...
namespace RenderLib
{
  Camera::Camera() : initWidth(-1), initHeight(-1)
  {
  ...
  void
  Camera::setProjectionParams(const FLOAT & nearPlane, const FLOAT & farPlane,
                              const FLOAT & FOV)
  {
  ...
  void
  Camera::initializeProjectionMatrix()
  {
    ...
    projectionMatrix(3, 2)
        = FLOAT(2.0) * nearPlane * farPlane / (nearPlane - farPlane);
    projectionMatrix(2, 3) = FLOAT(-1.0);
  }

  void
  Camera::initializeViewMatrix()
  {
    lookAt(VECTOR3(0, 0, 0), VECTOR3(0, 0, -1), VECTOR3(0, 1, 0));
  }

  void
  Camera::lookAt(const VECTOR3 & eye, const VECTOR3 & target,
                 const VECTOR3 & upVector)
  {
    ...
    viewMatrix << right.x(), right.y(), right.z(), -right.dot(eye), up.x(),
        up.y(), up.z(), -up.dot(eye), -forward.x(), -forward.y(), -forward.z(),
        forward.dot(eye), 0, 0, 0, 1;
  ...
  void
  Camera::translateView(const VECTOR3 & translation)
  {
  ...
  void
  Camera::rotateView(const VECTOR3 & angles)
  {
  ...
  void
  Camera::onWindowResize(const int & width, const int & height)
  {
    ...
    projectionMatrix(3, 2)
        = (FLOAT(-2.0) * nearPlane * farPlane) / (farPlane - nearPlane);
    projectionMatrix(2, 3) = FLOAT(-1.0);
  }
} // namespace RenderLib

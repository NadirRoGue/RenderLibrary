...
namespace RenderLib
{
  namespace GPU
  {
    namespace Mesh
    {
      ...
      GPUBuffer *
      GPUMeshManager::initializeStaticBuffer()
      {
      ...
      GPUBuffer *
      GPUMeshManager::initializeDynamicBuffer()
      {
      ...
      GPUBuffer *
      GPUMeshManager::initializePostProcessQuadBuffer()
      {
      ...
      GPUBuffer *
      GPUMeshManager::getStaticMeshBuffer()
      {
        return staticBuffer.get();
      }

      GPUBuffer *
      GPUMeshManager::getDynamicMeshBuffer()
      {
        return dynamicFrontBuffer.get();
      }

      GPUBuffer *
      GPUMeshManager::getSyncDynamicBuffer()
      {
        return dynamicBackBuffer.get();
      }

      GPUBuffer *
      GPUMeshManager::getPostProcessQuadBuffer()
      {
        return postProcessQuadBuffer.get();
      }

      void
      GPUMeshManager::generatePostProcessQuad()
      {
        unsigned int faces[] = { 0, 1, 2, 1, 3, 2 };
        ...
        ppqb->updateData(reinterpret_cast<char *>(faces), sizeof(faces),
        ...
        ppqm->vertices = GPUAttribute(
            sizeof(float) * 3, 3, 4, 0,
            sizeof(float) * 5, // Stride: 3 vertex coords + 2 uv coords
            GL_FLOAT, false);

        ppqm->uvs.resize(1);
        ppqm->uvs[0] = GPUAttribute(sizeof(float) * 2, 2, 4, sizeof(float) * 3,
                                    sizeof(float) * 5, GL_FLOAT, false);
      }

      void
      GPUMeshManager::swapDynamicBuffers()
      {
        dynamicFrontBuffer.swap(dynamicBackBuffer);
      }

      bool
      GPUMeshManager::meshAlreadyExists(size_t index, bool staticMesh)
      {
        return getGPUMesh(index, staticMesh) != NULL;
      }

      GPUMesh *
      GPUMeshManager::getGPUMesh(size_t index, bool staticMesh)
      {
        if (staticMesh)
        {
      ...
      GPUMesh *
      GPUMeshManager::createGPUMesh(size_t index, bool staticMesh)
      {
      ...
      GPUMesh *
      GPUMeshManager::getPostProcessQuad()
      {

...
namespace RenderLib
{
	namespace GPU
	{
		namespace Program
		{
			...
			void ComputeProgram::initialize(std::vector<std::string> & definesBuffer)
			{
				...
				compShader = loadShaderFromFile(GL_COMPUTE_SHADER, compShaderFile, definesBuffer);
				attachShader(compShader);
				link();
			}

			void ComputeProgram::destroyShaders()
			{

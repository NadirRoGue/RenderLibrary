...
namespace RenderLib
{
  namespace GPU
  {
    ...
    {
      inline void
      castDoubleToFloatArray(const size_t & count, const double * array,
      ...
      void
      Program::init(const UberParamMask & mask)
      {
        configMask = mask;
      ...
      void
      Program::attachShader(unsigned int shaderId)
      {
        if (shaderId != -1)
        {
      ...
      void
      Program::detachShader(unsigned int shaderId, bool deleteShader)
      {
        if (shaderId != -1)
        {
      ...
      unsigned int
      Program::loadShaderFromFile(GLenum shaderType,
                                  ...
                                  std::vector<std::string> & configStrings)
      {
        ...
        GPU::Program::ShaderLoadResult * loadResult
            = dynamic_cast<GPU::Program::ShaderLoadResult *>(
                fileloadResult.get());

        size_t fileLen      = loadResult->getResultSizeBytes() - 1;
        std::string defines = "";

        configStrings.emplace_back("MAX_DIRECTIONAL_LIGHTS "
                                   + std::to_string(MAX_DIRECTIONAL_LIGHTS));
        configStrings.emplace_back("MAX_POINT_LIGHTS "
                                   + std::to_string(MAX_POINT_LIGHTS));
        configStrings.emplace_back("MAX_SPOT_LIGHTS "
        ...
        glShaderSource(shader, 1, (const GLchar **)&finalSourceCStr,
                       (const GLint *)&fileLen);
        ...
        if (!compiled)
        {
          Logger::Log::getInstance().logError("Error compiling shader "
                                              + filePath);
          GLint logLen;
      ...
      Program::gatherInputs()
      {
        ...
        {
          glGetActiveUniform(programId, i, 0xff, &nameLen, &size, &type,
                             nameBuffer);
        ...
        {
          glGetActiveAttrib(programId, i, 0xff, &nameLen, &size, &type,
                            nameBuffer);
        ...
        while (i < activeBlocks)
        {
          ...
          glGetActiveUniformBlockiv(programId, i, GL_UNIFORM_BLOCK_NAME_LENGTH,
                                    &nameLen);
      ...
      void
      Program::getUberShaderDefines(std::vector<std::string> & definesBuffer)
      {
      }
      ...
      bool
      Program::hasMask(const UberParamMask & prop)
      {
        return (configMask & prop) == prop;
      }

      void
      Program::setUniformI(const std::string & name, const int & val)
      {
      ...
      void
      Program::setUniformI2(const std::string & name, const int & v1,
                            const int & v2)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
      ...
      void
      Program::setUniformI3(const std::string & name, const int & v1,
      ...
      void
      Program::setUniformI4(const std::string & name, const int & v1,
      ...
      void
      Program::setUniformI2V(const std::string & name,
                             const unsigned int & count, const int * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
        {
      ...
      void
      Program::setUniformI3V(const std::string & name,
                             const unsigned int & count, const int * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
      ...
      void
      Program::setUniformI4V(const std::string & name,
                             const unsigned int & count, const int * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
      ...
      void
      Program::setUniformF(const std::string & name, const FLOAT & val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
        {
      ...
      void
      Program::setUniform2F(const std::string & name, const FLOAT & v1,
                            const FLOAT & v2)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
      ...
      void
      Program::setUniform3F(const std::string & name, const FLOAT & v1,
...
#else
#ifdef USE_DOUBLE_PRECISSION__
        glUniform3f(id, static_cast<float>(v1), static_cast<float>(v2),
      ...
      void
      Program::setUniform4F(const std::string & name, const FLOAT & v1,
                            const FLOAT & v2, const FLOAT & v3,
                            const FLOAT & v4)
      {
        auto it = shaderUniforms.find(name);
...
#else
#ifdef USE_DOUBLE_PRECISSION__
        glUniform4f(id, static_cast<float>(v1), static_cast<float>(v2),
                    static_cast<float>(v3), static_cast<float>(v4));
#else
      ...
      void
      Program::setUniform2FV(const std::string & name,
                             const unsigned int & count, const FLOAT * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
        {
...
#if defined USE_DOUBLE_PRECISSION__                                            \
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniform3FV(const std::string & name,
                             const unsigned int & count, const FLOAT * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
    ...
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniform4FV(const std::string & name,
                             const unsigned int & count, const FLOAT * val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
    ...
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniformUI(const std::string & name, const unsigned int & val)
      {
        auto it = shaderUniforms.find(name);
        if (it == shaderUniforms.end())
        {
      ...
      void
      Program::setUniformUI2(const std::string & name, const unsigned int & v1,
      ...
      void
      Program::setUniformUI3(const std::string & name, const unsigned int & v1,
      ...
      void
      Program::setUniformUI4(const std::string & name, const unsigned int & v1,
                             const unsigned int & v2, const unsigned int & v3,
      ...
      void
      Program::setUniformUI2V(const std::string & name,
      ...
      void
      Program::setUniformUI3V(const std::string & name,
                              const unsigned int & count,
                              const unsigned int * val)
      {
        auto it = shaderUniforms.find(name);
      ...
      void
      Program::setUniformUI4V(const std::string & name,
                              const unsigned int & count,
                              const unsigned int * val)
      {
        auto it = shaderUniforms.find(name);
      ...
      void
      Program::setUniformMatrix2(const std::string & name,
                                 const unsigned int & count,
    ...
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniformMatrix3(const std::string & name,
                                 const unsigned int & count,
                                 const bool & transpose, const FLOAT * val)
      {
        auto it = shaderUniforms.find(name);
    ...
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniformMatrix4(const std::string & name,
                                 const unsigned int & count,
                                 const bool & transpose, const FLOAT * val)
      {
        auto it = shaderUniforms.find(name);
    ...
    && defined USE_DOUBLE_PRECISSION_ON_SHADERS__
      ...
      void
      Program::setUniformTexture(const std::string & name,
                                 ...
                                 unsigned int & textureUnit)
      {
        ...
        if (texture == NULL)
        {
          throw EngineException("Program " + std::string(typeid(*this).name())
      ...
      void
      Program::setUniformBlock(const std::string & name,
      ...
      void
      Program::configureShaderAttribute(const std::string & shaderAttribute,
                                        Mesh::GPUAttribute & meshAttribute)
      {
        if (meshAttribute.numElements > 0)
        {
          ...
          if (it != shaderAttributes.end())
          {
            ...
            glVertexAttribPointer(input.id, (GLsizei)meshAttribute.elementCount,

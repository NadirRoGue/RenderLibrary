  ...
  namespace GPU
  {
    namespace Program
    {
      std::unordered_map<std::type_index,
          ...
          ProgramManager::programFactories;
      ...
      Program *
      ProgramManager::getProgram(const UberParamMask configMask,
                                 const std::type_index programTypeId)
      {
        ...
        else
        {
          ...
          if (factoryIt == programFactories.end())
          {
            Logger::Log::getInstance().logWarning(
                "ProgramManager: Attempted to create an unregistered program: "
                + std::string(programTypeId.name()));
            return NULL;
          }

          std::unique_ptr<Program> newProgram
              = factoryIt->second.get()->createProgram(configMask);

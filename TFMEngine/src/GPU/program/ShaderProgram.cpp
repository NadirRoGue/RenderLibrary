...
namespace RenderLib
{
	namespace GPU
	{
		namespace Program
		{
			...
			std::unordered_map<UberParamMask, std::string> ShaderProgram::maskToDefines =
			{
			...
			{ ShaderProgram::HAS_TRANSPARENT_COLOR, "ENABLE_TRANSPARENT_COLOR" },

			{ ShaderProgram::HAS_OPACITY_VALUE, "ENABLE_OPACITY_VALUE" },
			{ ShaderProgram::HAS_SPECULAR_SCALE_VALUE, "ENABLE_SPECULAR_SCALE_VALUE" },
			{ ShaderProgram::HAS_SHININESS_VALUE, "ENABLE_SHININESS_VALUE" },
			{ ShaderProgram::HAS_IOR_VALUE, "ENABLE_IOR_VALUE"},

			{ ShaderProgram::HAS_NORMALMAP_TEXTURE, "ENABLE_NORMALMAP_TEXTURE" },
			{ ShaderProgram::HAS_HEIGHTMAP_TEXTURE, "ENABLE_HEIGHTMAP_TEXTURE"},
			{ ShaderProgram::HAS_DISPLACEMENTMAP_TEXTURE, "ENABLE_DISPLACEMENTMAP_TEXTURE"},
			...
			{ ShaderProgram::HAS_SHININESS_TEXTURE, "ENABLE_SHININESS_TEXTURE" },
			{ ShaderProgram::HAS_OPACITY_TEXTURE, "ENABLE_OPACITY_TEXTURE" },
			{ ShaderProgram::HAS_OTHER_TEXTURE, "ENABLE_OTHER_TEXTURE"},
			{ ShaderProgram::HAS_REFELCTIONMAP_TEXTURE, "ENABLE_REFELECTIONMAP_TEXTURE"},
			...
			void ShaderProgram::getUberShaderDefines(std::vector<std::string> & buffer)
			{
				//std::cout << std::endl;
			...
			void ShaderProgram::configureMeshParameters(GPU::Mesh::GPUMesh & gpuMesh)
			{
			...
			void ShaderProgram::sendMaterialParameters(const Material::Material & material)
			{
				...
				setUniform3FV("transparentColor", 1, material.transparentColor().data());
				...
				setUniformTexture("otherTexture",
					material.otherTexture.getTexture(), texUnit);
			}

			void ShaderProgram::sendTransformParameters(const SceneObject & object, const Camera & camera)
			{
				...
				setUniformMatrix4("model", 1, false, object.transform.worldModelMatrix.data());
			...
			void ShaderProgram::onRenderObject(DefaultImpl::MeshRenderer & renderable, const Camera & camera)
			{

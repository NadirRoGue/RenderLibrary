...
namespace RenderLib
{
  namespace GPU
  {
    namespace Program
    {
      std::unordered_map<std::type_index, std::unique_ptr<UberMaskFactory>>
          UberFactoryManager::factories;
      std::unique_ptr<UberMaskFactory> UberFactoryManager::defaultFactory;

      GPU::Program::UberParamMask
      UberFactoryManager::computeMask(DefaultImpl::MeshRenderer * renderable)
      {

...
namespace RenderLib
{
  ...
  {
    void
    CompactMeshBuilder::configureAttributes(
        CPU::Memory::MemoryBlock * memBlock, CPU::Mesh::Mesh * dst,
        CPU::Memory::BlockConfiguration * data)
    {
      ...
      if (!meshConfig)
      {
        throw std::runtime_error("CompactMeshBuilder: Wrong BlockConfiguration "
                                 "object passed to configure mesh attributes");
      }
      ...
      dst->faces.setAttributeSource(memBlock, offset, sizeof(IVECTOR3),
                                    meshConfig->numFaces);
      offset += meshConfig->numFaces * sizeof(IVECTOR3);

      const size_t verticesOffset = meshConfig->numVertices * sizeof(VECTOR3);

      dst->vertices.setAttributeSource(memBlock, offset, sizeof(VECTOR3),
                                       meshConfig->numVertices);
      offset += verticesOffset;

      if (meshConfig->hasNormals)
      {
        dst->normals.setAttributeSource(memBlock, offset, sizeV3,
                                        meshConfig->numVertices);
      ...
      {
        dst->tangents.setAttributeSource(memBlock, offset, sizeV3,
                                         meshConfig->numVertices);
        offset += (meshConfig->numVertices * sizeV3);
      }
      ...
      {
        dst->bitangents.setAttributeSource(memBlock, offset, sizeV3,
                                           meshConfig->numVertices);
        offset += (meshConfig->numVertices * sizeV3);
      }
      ...
      if (meshConfig->numUVChannels > 0)
      {
        ...
        {
          uvMap.setAttributeSource(memBlock, offset, sizeV2,
                                   meshConfig->numVertices);
      ...
      if (meshConfig->numColorChannels > 0)
      {
        ...
        {
          colorMap.setAttributeSource(memBlock, offset, sizeV4,
                                      meshConfig->numVertices);

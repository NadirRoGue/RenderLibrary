...
namespace RenderLib
{
  ...
  {
    void
    CPUToGPUMeshSyncStage::preRunStage()
    {
    ...
    void
    CPUToGPUMeshSyncStage::runStage()
    {
    ...
    void
    CPUToGPUMeshSyncStage::registerMeshes(SourceMeshList & syncOnce,
                                          SourceMeshList & syncContinously)
    {
      if (elements.size() > 0)
      {
        for (auto comp : elements)
        {
          ...
          if (renderable->cpuToGpuSync
                  == CPUToGPUSyncPolicy::CPU_SYNC_ONCE_AT_BEGINNING
              && renderable->gpuToCpuSync
                  == GPUToCPUSyncPolicy::GPU_DO_NOT_SYNC)
          {
    ...
    void
    CPUToGPUMeshSyncStage::createGPUMeshes(SourceMeshList & src,
                                           GPUMeshList & dst, bool staticMeshes)
    {
      ...
      for (auto mesh : src)
      {
        ...
        GPU::Mesh::GPUMesh * gpuMesh
            = engineInstance->getGPUMeshManager().getGPUMesh(cpuMesh->index,
                                                             staticMeshes);
        if (gpuMesh == NULL)
        {
          gpuMesh
              = buildGPUMeshFromCPUMesh(cpuMesh, prevFaceSize, staticMeshes);
    ...
    GPU::Mesh::GPUMesh *
    CPUToGPUMeshSyncStage::buildGPUMeshFromCPUMesh(CPU::Mesh::Mesh * cpuMesh,
                                                   size_t faceOffsetFix,
                                                   bool staticMesh)
    {
      GPU::Mesh::GPUMesh * gpuMesh
          = engineInstance->getGPUMeshManager().createGPUMesh(cpuMesh->index,
                                                              staticMesh);

      size_t baseOffset
          = cpuMesh->vertices.getMemorySource()->offset - faceOffsetFix;
      ...
      gpuMesh->faces = GPU::Mesh::GPUAttribute(
          cpuMesh->faces.getElementSize(), 3, cpuMesh->faces.size(),
          baseOffset + cpuMesh->faces.getOffset(), cpuMesh->faces.getStride(),
          GL_UNSIGNED_INT, false);

      gpuMesh->vertices = GPU::Mesh::GPUAttribute(
          cpuMesh->vertices.getElementSize(), 3, cpuMesh->vertices.size(),
          baseOffset + cpuMesh->vertices.getOffset() - vOffset,
          cpuMesh->vertices.getStride(), GL_FLOAT, false);

      gpuMesh->normals = GPU::Mesh::GPUAttribute(
          cpuMesh->normals.getElementSize(), v3NumElements,
          cpuMesh->normals.size(),
          baseOffset + cpuMesh->normals.getOffset() - vOffset,
          cpuMesh->normals.getStride(), v34DataType, normalize);

      gpuMesh->tangents = GPU::Mesh::GPUAttribute(
          cpuMesh->tangents.getElementSize(), v3NumElements,
          cpuMesh->tangents.size(),
          baseOffset + cpuMesh->tangents.getOffset() - vOffset,
          cpuMesh->tangents.getStride(), v34DataType, normalize);

      gpuMesh->bitangents = GPU::Mesh::GPUAttribute(
          cpuMesh->bitangents.getElementSize(), v3NumElements,
          cpuMesh->bitangents.size(),
          baseOffset + cpuMesh->bitangents.getOffset() - vOffset,
          cpuMesh->bitangents.getStride(), v34DataType, normalize);

      gpuMesh->uvs.resize(cpuMesh->uvs.size());
      size_t i = 0;
      for (auto & uvAttrib : cpuMesh->uvs)
      {
        gpuMesh->uvs[i] = GPU::Mesh::GPUAttribute(
            uvAttrib.getElementSize(), v2NumElements, uvAttrib.size(),
            baseOffset + uvAttrib.getOffset() - vOffset, uvAttrib.getStride(),
            v2DataType, normalize);
        i++;
      }
      ...
      {
        gpuMesh->colors[i] = GPU::Mesh::GPUAttribute(
            colorAttrib.getElementSize(), v4NumElements, colorAttrib.size(),
            baseOffset + colorAttrib.getOffset() - vOffset,
            colorAttrib.getStride(), v34DataType, normalize);
        i++;
      }
    ...
    void
    CPUToGPUMeshSyncStage::synchronizeData(GPU::Mesh::GPUBuffer * buffer,
                                           GPUMeshList & sourceMeshes)
    {
      ...
      for (auto & data : sourceMeshes)
      {
        ...
        // Copy indexes
        memcpy(faceBuffer + facesOffset, poolData + block->offset,
               data.facesSize);
        // Copy data
        memcpy(dataBuffer + dataOffset,
               poolData + block->offset + data.facesSize, data.dataSize);

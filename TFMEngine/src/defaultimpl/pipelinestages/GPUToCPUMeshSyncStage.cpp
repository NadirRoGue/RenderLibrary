...
namespace RenderLib
{
  ...
  {
    void
    GPUToCPUMeshSyncStage::registerElement(Component * comp)
    {
    ...
    void
    GPUToCPUMeshSyncStage::runStage()
    {
      updateCPUMeshes(syncContinousMeshes);
    }

    void
    GPUToCPUMeshSyncStage::postRunStage()
    {
      updateCPUMeshes(syncOnceMeshes);
    }

    void
    GPUToCPUMeshSyncStage::updateCPUMeshes(
        std::vector<GPU::Mesh::GPUMesh *> & list)
    {
      ...
      char * data = reinterpret_cast<char *>(
          glMapBufferRange(GL_ARRAY_BUFFER, 0,
                           dynamicBuffer->getDataSizeBytes(), GL_MAP_READ_BIT));
      ...
      if (memPool == NULL)
      {
        throw EngineException("GPUToCPUSyncStage: No memory pool for "
                              "CPU::Mesh::Mesh found. Aborting sync");
      }
      ...
      for (auto gpuMesh : list)
      {
        ...
        if (blockIt != blockMap.end())
        {
          ...
          memPool->setBlockData(block, gpuMesh->faceIndexOffset,
                                gpuMesh->dataSize,

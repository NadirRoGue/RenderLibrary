...
namespace RenderLib
{
  ...
  {
    void
    ShaderCompilationStage::preRunStage()
    {
    ...
    void
    ShaderCompilationStage::runStage()
    {
    ...
    void
    ShaderCompilationStage::compileShaders()
    {
      if (elements.size() > 0)
      {
        for (auto component : elements)
        {
          ...
          GPU::Program::UberParamMask mask
              = GPU::Program::UberFactoryManager::computeMask(renderable);

          // Compile if not done already. Return already compiled shader if present
          GPU::Program::Program * prog
              = engineInstance->getProgramManager().getProgram(
                  mask, renderable->shaderType);

...
namespace RenderLib
{
	namespace DefaultImpl
	{
		...
		void BloomProgram::initializeShader(EngineInstance * instance)
		{
			...
			fbos[0].input = 
				fbos[0].fbo->addColorAttachment(0, "emissiveBuffer_0", GL_RGBA16F, GL_RGBA, GL_FLOAT);
			...
			fbos[1].input = 
				fbos[1].fbo->addColorAttachment(0, "emissiveBuffer_1", GL_RGBA16F, GL_RGBA, GL_FLOAT);
			fbos[1].input->getConfig().setMinMaxFilter(GL_LINEAR, GL_LINEAR);
			fbos[1].fbo->setSize(500, 500);
		}

		void BloomProgram::onRender(GPU::Texture::GPUTexture * previousOutput, EngineInstance * instance)
		{

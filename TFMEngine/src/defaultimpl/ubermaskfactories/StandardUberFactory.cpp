...
namespace RenderLib
{
  namespace DefaultImpl
  {
    ...
    StandardUberFactory::computeMask(DefaultImpl::MeshRenderer * renderable)
    {
      ...
      mask |= (renderable->preferredRender == DefaultImpl::FORWARD_RENDER
               || renderable->material->opacity() < 1.0f)
          ? GPU::Program::ShaderProgram::FORWARD_SHADING
          : 0;
    ...
    GPU::Program::UberParamMask
    StandardUberFactory::computeMaskForMesh(GPU::Mesh::GPUMesh * mesh)
    {
      if (mesh == NULL)
      {
        return 0;
    ...
    GPU::Program::UberParamMask
    StandardUberFactory::computeMaskForMaterial(Material::Material * material)
    {
      if (material == NULL)
      {
        return 0;
      ...
      materialMask |= material->ambientColor.paramExists()
          ? GPU::Program::ShaderProgram::HAS_AMBIENT_COLOR
          : 0;
      materialMask |= material->diffuseColor.paramExists()
          ? GPU::Program::ShaderProgram::HAS_DIFFUSE_COLOR
          : 0;
      materialMask |= material->specularColor.paramExists()
          ? GPU::Program::ShaderProgram::HAS_SPECULAR_COLOR
          : 0;
      materialMask |= material->emissiveColor.paramExists()
          ? GPU::Program::ShaderProgram::HAS_EMISSIVE_COLOR
          : 0;

      materialMask |= material->shininess.paramExists()
          ? GPU::Program::ShaderProgram::HAS_SHININESS_VALUE
          : 0;
      materialMask
          |= material->opacity.paramExists() && material->opacity() < 1.0f
          ? GPU::Program::ShaderProgram::HAS_OPACITY_VALUE
          : 0;
      materialMask |= material->specularScale.paramExists()
          ? GPU::Program::ShaderProgram::HAS_SPECULAR_SCALE_VALUE
          : 0;
      materialMask |= material->indexOfRefraction.paramExists()
          ? GPU::Program::ShaderProgram::HAS_IOR_VALUE
          : 0;
      ...
      {
        materialMask
            |= GPU::Program::ShaderProgram::HAS_DISPLACEMENTMAP_TEXTURE;
      }

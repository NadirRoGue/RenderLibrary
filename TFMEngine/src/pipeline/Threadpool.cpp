  ...
  namespace Pipeline
  {
    ...
    ThreadPool::ThreadPool()
    {
      ...
      Logger::Log::getInstance().logInfo(
          "ThreadPool: Using " + std::to_string(poolSize) + " thread(s)");
    }

    void
    ThreadPool::init(int threadPoolSize)
    {
      active = true;
      if (threadPoolSize < 1)
      {
    ...
    void
    ThreadPool::addTask(std::unique_ptr<Runnable> & task)
    {
      std::unique_lock<std::mutex> lock(globalLock);
    ...
    void
    ThreadPool::processStage(AbstractElementBasedStage & stage)
    {
      activeThreads = 0;
      ...
      while (assigned < elementsSize)
      {
        ...
        size_t blockOffset
            = std::min<size_t>(assigned + perThreadAmount, elementsSize);
        newTask.get()->elementsToProcess.assign(itBegin + assigned,
                                                itBegin + blockOffset);

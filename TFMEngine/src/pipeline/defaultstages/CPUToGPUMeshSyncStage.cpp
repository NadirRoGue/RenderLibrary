...
namespace RenderLib
{
	namespace Pipeline
	{
		void CPUToGPUMeshSyncStage::preRunStage()
		{
			...
			ElementBasedStage<Components::MeshRenderer>::preRunStage();
		...
		void CPUToGPUMeshSyncStage::registerMeshes()
		{
			if (elements.size() > 0)
			{
				...
				{
					Components::MeshRenderer * renderable = static_cast<Components::MeshRenderer*>(comp);
		...
		void CPUToGPUMeshSyncStage::createGPUMeshes(SourceMeshList & src, GPUMeshList & dst, size_t & faceSizeBytes, size_t & dataSizeBytes, bool staticMeshes)
		{
			...
			for (auto mesh : src)
			{
				...
				if (gpuMesh == NULL)
				{
					...
					data.facesSize = gpuMesh->faces.elementSize * gpuMesh->faces.elementCount;

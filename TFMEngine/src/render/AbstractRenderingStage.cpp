...
namespace RenderLib
{
  namespace Render
  {
    AbstractRenderingStage::AbstractRenderingStage()
      : outputFBO(NULL), prevStage(NULL), nextStage(NULL)
    {
    }

    GPU::Texture::GPUTexture *
    AbstractRenderingStage::getStageOutput()
    {
      return finalOutput;
    }

    void
    AbstractRenderingStage::initialize()
    {
    }

    void
    AbstractRenderingStage::finalize()
    {
    }

    void
    AbstractRenderingStage::tryRegisterElement(
        DefaultImpl::MeshRenderer * renderable)
    {
    }

    void
    AbstractRenderingStage::forceRegisterRenderable(
        DefaultImpl::MeshRenderer * renderable)
    {
    }

    void
    AbstractRenderingStage::setTargetFBO(FBO * fbo)
    {
      outputFBO = fbo;
    }
    ...
    RenderableMap &
    MeshRenderingStage::getStaticRenderables()
    {
      return staticRenderables;
    }

    RenderableMap &
    MeshRenderingStage::getDynamicRenderables()
    {
    ...
    MeshRenderingStage::initialize()
    {
      const std::string name
          = std::to_string(engineInstance->getInstanceID()) + "_coloroutput";
    ...
    void
    MeshRenderingStage::tryRegisterElement(
        DefaultImpl::MeshRenderer * renderable)
    {
      if (shouldRegisterRenderable(renderable))
      {
        ...
        {
          dynamicRenderables.addRenderable(engineInstance->getProgramManager(),
                                           renderable);
        }
        else
        {
          staticRenderables.addRenderable(engineInstance->getProgramManager(),
                                          renderable);
        }
      }
    }
    ...
    void
    MeshRenderingStage::forceRegisterRenderable(
        DefaultImpl::MeshRenderer * renderable)
    {
      if (renderable->cpuToGpuSync == DefaultImpl::CPU_SYNC_CONTINOUSLY
          || renderable->gpuToCpuSync == DefaultImpl::GPU_SYNC_CONTINOUSLY)

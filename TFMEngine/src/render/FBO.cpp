...
namespace RenderLib
{
  namespace Render
  {
    ...
    void
    FBO::initializeDefaultFBO()
    {
      DEFAULTFRAMEBUFFER.id = 0;
    }

    void
    FBO::queryMaxRenderTargets()
    {
    ...
    void
    FBO::setEngineInstance(EngineInstance * instance)
    {
      accessor = instance;
    }
    ...
    FBO::addColorAttachment(const unsigned int & index,
                            ...
                            GLenum format, GLenum pixelType)
    {
      if (index > MAX_RENDER_TARGETS)
      {
        throw EngineException(
            "FBO: Adding color attachment failed. Maximun attachments are "
            + std::to_string(MAX_RENDER_TARGETS));
        return NULL;
      }

      GPU::Texture::GPUTexture * colorAttachment
          = accessor->getGPUTextureManager()
    ...
    GPU::Texture::GPUTexture *
    FBO::addDepthAttachment(const std::string & name)
    {
      ...
      GPU::Texture::GPUTextureConfig & config
          = depthAttachment.attachment->getConfig();
    ...
    GPU::Texture::GPUTexture *
    FBO::getAttachment(const std::string & name)
    {
    ...
    void
    FBO::setSize(const unsigned int & width, const unsigned int & height)
    {
      this->width  = width;
      this->height = height;
      ...
      for (auto & info : indexedAttachments)
      {
        if (info.attachment != NULL)
        {
          glFramebufferTexture2D(GL_FRAMEBUFFER, info.attachmentType,
                                 GL_TEXTURE_2D, info.attachment->getId(), 0);
          attachmentList.push_back(info.attachmentType);
        }
      }

      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                             depthAttachment.attachment->getId(), 0);
      ...
      {
        throw EngineException("FBO: Incomplete framebuffer: Error code: "
                              + std::to_string(error));
      }

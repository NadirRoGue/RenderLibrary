...
namespace RenderLib
{
	namespace Render
	{
		...
		void DeferredRenderStage::initialize()
		{
			...
			GPU::Texture::GPUTexture * diffuseAndOpacity = gBuffers->addColorAttachment(
				1, 
				"diffuseAndOpacity",
				GL_RGBA16F,
				GL_RGBA,
			...
			GPU::Texture::GPUTexture * specularAndShininess = gBuffers->addColorAttachment(
				3, 
				"specularAndShininess",
				GL_RGBA16F,
				GL_RGBA,
				GL_FLOAT);
		...
		void DeferredRenderStage::runStage()
		{
			...
			GPU::Mesh::GPUBuffer * postProcessBuf = engineInstance->getGPUMeshManager().getPostProcessQuadBuffer();
			postProcessBuf->bind();
